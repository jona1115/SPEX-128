...
module fixed128_partitione_unit_test;
  ...
  string name = "fixed128_partitione_ut";
...
initial begin
  ...
  localparam string LUT_FILE = my_fixed128_partitione.INIT_FILE; // follow DUT param
  binary128_t gt_mem [0:8191];

  initial begin
    $display("TB: loading LUT file: %s", LUT_FILE);
    $readmemh(LUT_FILE, gt_mem);
  end

  task automatic drive(input logic [12:0] e, input bit v);
    s_i_e     = e;
    s_i_valid = v;
  endtask
  ...
  task automatic expect_now_valid_and_value(input logic [12:0] e, input string tag = "");
    ...
    `FAIL_IF_LOG(s_o_exp_e !== gt_mem[e],
                $sformatf(">>>>> %s: LUT mismatch for d=%0d (0x%0h)", tag, e, e))
  ...
  task automatic expect_hold(input binary128_t exp, input string tag = "");
    `FAIL_IF_LOG(s_o_valid !== 1'b0,
                $sformatf(">>>>> %s: expected o_valid=0 during bubble, got %0b", tag, s_o_valid))
    `FAIL_IF_LOG(s_o_exp_e !== exp,
