...
module fixed128_partitiond_unit_test;
  ...
  string name = "fixed128_partitiond_ut";
...
initial begin
  ...
  task automatic expect_now_valid_and_value(input logic [12:0] d, input string tag = "");
    ...
    `FAIL_IF_LOG(s_o_exp_d !== gt_mem[d],
                $sformatf(">>>>> %s: LUT mismatch for d=%0d (0x%0h)", tag, d, d))
