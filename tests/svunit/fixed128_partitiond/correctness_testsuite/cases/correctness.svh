...
`SVTEST(correctness_reset_state)
  ...
  `FAIL_IF_LOG(s_o_exp_d !== '0,   ">>>>> o_exp_d not 0 after reset")
...
`SVTEST(correctness_single_pulse)
  logic [12:0] d = 13'h0123;
  drive(d, 1'b1);
  tick(); // registered BRAM + valid
  expect_now_valid_and_value(d, "single_pulse");

  drive('0, 1'b0);
  tick();
  expect_hold(gt_mem[d], "single_pulse_hold");
`SVTEST_END
...
`SVTEST(correctness_random_20)
  ...
  for (int i = 0; i < N; i++) begin
    logic [12:0] d = $urandom_range(0, 8191);
    drive(d, 1'b1); tick(); expect_now_valid_and_value(d, $sformatf("rand[%0d]", i));
    drive('0, 1'b0); tick(); expect_hold(gt_mem[d], $sformatf("rand_hold[%0d]", i));
  end
`SVTEST_END

`SVTEST(correctness_sync_reset_midstream)
  logic [12:0] d  = 13'h07FF;
  logic [12:0] d2 = 13'h1A2B;

  drive(d, 1'b1); tick(); expect_now_valid_and_value(d, "pre_reset");

  s_i_reset = 1'b0;  drive('0, 1'b0); tick(); // sync reset tick
  `FAIL_IF_LOG(s_o_valid !== 1'b0, ">>>>> o_valid not 0 on sync reset")
  `FAIL_IF_LOG(s_o_exp_d !== '0,   ">>>>> o_exp_d not 0 on sync reset")
