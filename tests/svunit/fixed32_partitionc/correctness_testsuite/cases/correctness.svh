...
`SVTEST(correctness_reset_state)
  ...
  `FAIL_IF_LOG(s_o_exp_c !== '0,   ">>>>> o_exp_a not 0 after reset")
...
`SVTEST(correctness_bubble_and_hold_positive)
  ...
  logic [10:0] d1 = 11'b11_1010_1010;
  drive(d0, 1'b1); tick(); expect_now_valid_and_value(d0, "bubble_d0 pos");
  // $display("+++++ s_o_exp_c=0x%x", s_o_exp_c);
...
`SVTEST(correctness_random_20)
  ...
  for (int i = 0; i < N; i++) begin
    logic [10:0] c = $urandom_range(0, 2047);
    drive(c, 1'b1); tick(); expect_now_valid_and_value(c, $sformatf("rand[%0d]", i));
    drive('0, 1'b0); tick(); expect_hold(gt_mem[c], $sformatf("rand_hold[%0d]", i));
  end
`SVTEST_END
