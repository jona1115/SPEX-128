...
module fixed32_partitionc_unit_test;
  ...
  string name = "fixed32_partitionc_ut";
...
initial begin
  ...
  localparam string LUT_FILE = my_fixed32_partitionc.INIT_FILE;
  ...
  task automatic drive(input logic [10:0] c, input bit v);
    s_i_c     = c;
    s_i_valid = v;
  endtask
   ...
   */
  task automatic expect_now_valid_and_value(input logic [10:0] c, input string tag = "");
    `FAIL_IF_LOG(s_o_valid !== 1'b1,
                $sformatf(">>>>> %s: expected o_valid=1, got %0b @%0t", tag, s_o_valid, $time))
    `FAIL_IF_LOG(s_o_exp_c !== gt_mem[c],
                $sformatf(">>>>> %s: LUT mismatch for c=%0d (0x%0h), expected:0x%0h, got:0x%0h", 
                  tag, c, c, gt_mem[c], s_o_exp_c))
